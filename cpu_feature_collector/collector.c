...
bool is_isa_set_supported(xed_isa_set_enum_t isa_set) {
    // I86, 386 등 초창기 ISA는 CPUID 개념 이전에 나왔으므로 항상 지원된다고 가정합니다.
    // XED는 이들에 대해 CPUID 그룹을 반환하지 않습니다.
    if (isa_set == XED_ISA_SET_I86 || isa_set == XED_ISA_SET_I186 || 
        isa_set == XED_ISA_SET_I286REAL || isa_set == XED_ISA_SET_I286PROTECTED ||
        isa_set == XED_ISA_SET_I386) {
        return true;
    }
    ...
    for (int i = 0; i < XED_MAX_CPUID_GROUPS_PER_ISA_SET; ++i) {
        ...
        if (group == XED_CPUID_GROUP_INVALID) {
            break; // 이 ISA-SET에 대한 더 이상의 그룹은 없음
        }
        ...
        for (int j = 0; j < XED_MAX_CPUID_RECS_PER_GROUP; ++j) {
            ...
            switch(crec.reg) {
                case XED_REG_EAX: host_reg_val = eax; break;
                case XED_REG_EBX: host_reg_val = ebx; break;
                case XED_REG_ECX: host_reg_val = ecx; break;
                case XED_REG_EDX: host_reg_val = edx; break;
                default:
                    fprintf(stderr, "Error: Unexpected register in CPUID record.\n");
                    group_supported = false;
                    break;
            }
            if (!group_supported) break;
