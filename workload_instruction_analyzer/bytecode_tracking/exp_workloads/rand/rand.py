...
librand = ctypes.CDLL(
    f'/home/ubuntu/LiveMigrate-Detector/workload_instruction_analyzer/bytecode_tracking/exp_workloads/rand/librand.so')
...
while (True):
    ...
    if is_rdseed_support:
        ...
        print(
            f"Generated random number using RDSEED: {random_number}", flush=True)
    else:
        random_number = generate_random_number_rdrand()
        print(
            f"Generated random number using RDRAND: {random_number}", flush=True)
