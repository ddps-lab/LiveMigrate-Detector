...
print(f"PKU supported: {pku_supported}")
...
while(True):
    ...
    print(f"Buffer address: {buffer_address:#x}")  # 메모리 주소를 16진수로 출력

    # 5. 페이지 정렬 확인
    if buffer_address % PAGE_SIZE != 0:
        raise RuntimeError(f"Buffer address is not aligned: {buffer_address:#x}")

    # 6. 메모리에 값을 씀
    value_to_write = b'A'
    print(f"Attempting to write value '{value_to_write.decode()}' to memory at address {buffer_address:#x}")
    pku.write_memory(value_to_write, buffer_address)
    print(f"Value written to memory: {value_to_write.decode()}")

    # 7. 메모리를 보호
    if pku_supported:
        pku.protect_memory_pku(buffer_address)
        print("Memory protected using PKU.")
    else:
        pku.protect_memory_mprotect(buffer_address)
        print("Memory protected using mprotect.")
    ...
    print("Memory protection lifted.")

    # 10. 보호 해제된 메모리에 값을 다시 씀
    pku.write_memory(b'C', buffer_address)
    print("Value written to memory after unprotecting: C")

    # 11. 메모리에서 값을 읽음
    value_read = pku.read_memory(buffer_address)
    print(f"Value read from memory: {value_read.decode()}")
