...
libxedwrapper = ctypes.CDLL(
    '/home/ubuntu/LiveMigrate-Detector/workload_instruction_analyzer/xedlib/libxedwrapper.so')
...
def disas(start_addr, end_addr, seen, buffered_output, name):
    ...
    progress = tqdm(total=end_addr-start_addr,
                    desc=f'{name}', unit="B", mininterval=10)
    ...
    while current_addr <= end_addr:
        ...
        if instruction_hex not in seen:
            buffered_output.append(
                f"{hex(instruction.address)}: {instruction.mnemonic} {instruction_hex}\n")
...
if __name__ == '__main__':
    ...
    dis_time = time.time() - dis_start_time
